...
async function openVideo(v) {
  ...
  // ✅ Search History hide when video is opened
  document.getElementById("searchHistoryContainer").classList.add("hidden");
...
function openChannel(uid, name, logo) {
  ...
  // ✅ Search History hide on Channel open
  document.getElementById("searchHistoryContainer").classList.add("hidden");
...
// ===============================
// ✅ SEARCH BUTTON — Perform search + Hide history
searchBtn.onclick = async () => {
  const q = searchInput.value.trim().toLowerCase();
  if (!q) return;

  // ✅ Save search uniquely
  if (currentUserId) {
  ...
  // ✅ Search history hide on search click
  document.getElementById("searchHistoryContainer").classList.add("hidden");
...
// ✅ Show Search History ONLY on input click
searchInput.onclick = () => {
  if (!currentUserId) return;
  loadSearchHistory();
  document.getElementById("searchHistoryContainer").classList.remove("hidden");
};
// ===============================
